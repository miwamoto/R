...
```{R}
model3 <- lm(bbscore ~ npvis + cigs + drink + mage + male + feduc + fwhte + meduc + mwhte + monpre, data = data)
model3
```
...
#Conclusion
In conclusion, we can state that prenatal health care measures are generally associated with healthier babies. However, other variables also showed correlation with positive health outcomes.
